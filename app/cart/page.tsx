  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        {/* Group cart items by table for better organization */}
        {Object.entries(
          ...
          <div key={table} className="mb-8 p-4 border rounded-md bg-gray-50">
            <h3 className="text-xl font-bold mb-4 text-gray-700">Order for {table}</h3>
            {items.map((item) => (
              <div key={`${item.id}-${JSON.stringify(item.options)}-${item.table}`} className="flex items-center justify-between border-b last:border-b-0 py-4">
                ...
                <div className="flex items-center space-x-3">
                  <button
                    onClick={() => updateQuantity(item.id, item.quantity - 1, item.options, item.table)}
                  ...
                  <button
                    onClick={() => removeItem(item.id, item.options, item.table)}
        ...
        <div className="flex justify-between items-center mt-6 pt-4 border-t-2 border-gray-200">
          ...
          <span className="text-2xl font-extrabold text-green-700">${getTotalPrice().toFixed(2)}</span>
        </div>
